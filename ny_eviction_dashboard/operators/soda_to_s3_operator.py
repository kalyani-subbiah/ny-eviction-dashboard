...
class SodaToS3Operator(BaseOperator):
	"""
	Queries the Socrata Open Data API using SoQL string and uploads the results to an S3 bucket.

	:param endpoint:    Optional API connection endpoint
	:param data:    Custom Socrata SoQL string used to query API, overrides the default get request
	...
	def get_size(self, obj, seen=None):
		"""
		Recursively find size of object
		"""
		...
		if isinstance(obj, dict):
			...
			size += sum([self.get_size(v, seen) for v in obj.keys()])
	...
	def parse_metadata(self, header):
		"""
		Parses metadata from API response. Writes to logs folder.
		"""
		try:
			metadata = {
	...
	def execute(self, context):
		...
		The SODA API maxes out the # of returned results so we use paging to query the endpoint
		multiple times and continuously move the offset. 

		Metadata is parsed and saved separately in a /logs/ subfolder along with the JSON
